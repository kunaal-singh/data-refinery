...
export default function App() {
  ...
  return <div className={data ? 'app-shell workspace-shell' : 'app-shell landing-shell'}><header className="topbar"><a className="brand" href="#" onClick={e => {
        e.preventDefault();
        setData(null);
      }}><span className="brand-logo-frame"><img className="brand-logo" src="/datarefinery-logo.png" alt="DataRefinery" /></span></a><nav className="topnav">{data ? nav.map(({name}) => <button className={tab === name ? 'nav-link active' : 'nav-link'} onClick={() => setTab(name)} key={name}>{name}</button>) : <><button className="nav-link" onClick={() => document.getElementById('how-it-works')?.scrollIntoView({behavior:'smooth'})}>How it works</button><button className="nav-link" onClick={() => document.getElementById('features')?.scrollIntoView({behavior:'smooth'})}>Features</button><button className="nav-link" onClick={() => document.getElementById('sample-data')?.scrollIntoView({behavior:'smooth'})}>Sample data</button><button className="landing-nav-cta" onClick={() => input.current?.click()}>Upload data <ArrowRight size={15}/></button></>}</nav></header><main className="main-area">{!data ? <LandingPage input={input} busy={busy} error={error} pendingWorkbook={pendingWorkbook} selectedSheet={selectedSheet} setSelectedSheet={setSelectedSheet} sampleName={sampleName} setSampleName={setSampleName} sample={sample} savedDatasets={savedDatasets} reopen={reopen} upload={upload} /> : <Workspace data={data} setData={setData} syncWarning={syncWarning} setSyncWarning={setSyncWarning} setTab={setTab} tab={tab} onReset={() => setData(null)} query={query} setQuery={setQuery} answer={answer} setAnswer={setAnswer} />}</main><footer className="footer"><span>{"\u2733 \u00A0DataRefinery"}</span><span>{"Clearer answers start with better questions."}</span><button onClick={() => data ? setData(null) : window.scrollTo({
        top: 0,
        behavior: 'smooth'
...
const sampleOptions = [
  {id:'sales',title:'Sales',subtitle:'Revenue and product trends',icon:BarChart3},
  {id:'ecommerce',title:'E-commerce',subtitle:'Orders, channels, and revenue',icon:Table2},
  {id:'marketing',title:'Marketing',subtitle:'Campaign spend and results',icon:Sparkles},
  {id:'web_analytics',title:'Web analytics',subtitle:'Traffic and engagement',icon:Search},
  {id:'inventory',title:'Inventory',subtitle:'Stock and category levels',icon:FileSpreadsheet}
];
function LandingPage({input,busy,error,pendingWorkbook,selectedSheet,setSelectedSheet,sampleName,setSampleName,sample,savedDatasets,reopen,upload}) {
  const [preview,setPreview]=useState(null);
  useEffect(()=>{let active=true;fetch('/samples/sales.csv').then(response=>{if(!response.ok)throw new Error('Sample preview unavailable');return response.text()}).then(text=>{const parsed=Papa.parse(text,{header:true,dynamicTyping:true,skipEmptyLines:true});if(active)setPreview(profile('Sales sample',parsed.data))}).catch(()=>{});return()=>{active=false}},[]);
  const previewCharts=preview?suggestions(preview).filter(chart=>chart.kind==='line'||chart.kind==='bar').slice(0,2):[];
  return <div className="landing-page">
    <section className="landing-hero"><div className="eyebrow"><span/>DATA VISUALIZATION & ANALYTICS</div><h1>Turn messy data into<br/><span>clear dashboards.</span></h1><p>Upload CSV, Excel, or JSON files. DataRefinery profiles your columns, flags data quality issues, and recommends useful charts.</p><div className="landing-hero-actions"><button className="landing-primary" disabled={busy} onClick={()=>input.current?.click()}><Upload size={17}/>{busy?'Analyzing your data…':'Upload your data'}</button><button className="landing-secondary" onClick={()=>sample('sales')}>Explore sample data <ArrowRight size={16}/></button></div></section>
    <section className="landing-uploader" onDragOver={event=>event.preventDefault()} onDrop={event=>{event.preventDefault();upload(event.dataTransfer.files?.[0])}}><div className="upload-card-heading"><span><FileUp size={20}/></span><div><h2>{busy?'Analyzing your dataset…':'Upload a data file'}</h2><p>{busy?'Reading columns and finding useful patterns':'Choose a file or drop it in the area below to begin'}</p></div></div><div className="upload-dropzone" onDragOver={event=>event.preventDefault()} onDrop={event=>{event.preventDefault();upload(event.dataTransfer.files?.[0])}}><FileSpreadsheet size={36}/><strong>Drag and drop your file here</strong><span>or</span><button className="landing-primary" disabled={busy} onClick={()=>input.current?.click()}><Upload size={16}/>{busy?'Working…':'Choose a file'}</button><div className="file-types"><span>CSV</span><span>XLSX</span><span>JSON</span><small>Up to 50 MB</small></div></div>{pendingWorkbook&&<div className="sheet-picker"><label>Choose a worksheet in {pendingWorkbook.file.name}<select value={selectedSheet} onChange={event=>setSelectedSheet(event.target.value)}><option value="">Select a sheet…</option>{pendingWorkbook.sheets.map(sheet=><option key={sheet}>{sheet}</option>)}</select></label><button className="landing-primary" disabled={!selectedSheet||busy} onClick={()=>upload(pendingWorkbook.file,selectedSheet)}>Analyze sheet <ArrowRight size={14}/></button></div>}{error&&<div className="error-message">{error}</div>}<p className="privacy-note"><Check size={14}/>Your files stay in your local workspace. No account needed.</p></section>
    <input ref={input} className="hidden-input" type="file" accept=".csv,.tsv,.txt,.xlsx,.xlsm,.json" onChange={event=>{upload(event.target.files?.[0]);event.target.value=''}}/>
    <section id="how-it-works" className="landing-section"><div className="landing-section-heading"><span>THREE SIMPLE STEPS</span><h2>From file to insight</h2><p>Start with the data you have. Explore the patterns DataRefinery finds.</p></div><div className="landing-steps"><article><span className="landing-step-icon"><Upload size={23}/></span><small>01</small><h3>Upload a dataset</h3><p>Drop in a CSV, Excel, or JSON file. Delimiters, encodings, and usable workbook sheets are handled for you.</p></article><article><span className="landing-step-icon"><Sparkles size={23}/></span><small>02</small><h3>Review the analysis</h3><p>See detected column types, useful chart ideas, summary statistics, and data quality findings.</p></article><article><span className="landing-step-icon"><LayoutDashboard size={23}/></span><small>03</small><h3>Build and export</h3><p>Customize charts, assemble a dashboard, explore rows, and export your results.</p></article></div></section>
    <section id="sample-data" className="landing-samples"><div className="landing-section-heading"><span>READY TO EXPLORE</span><h2>Try sample data</h2><p>Load one of the synthetic datasets included with the project.</p></div><div className="sample-grid">{sampleOptions.map(({id,title,subtitle,icon:Icon})=><button className={sampleName===id?'sample-card selected':'sample-card'} key={id} disabled={busy} onClick={()=>{setSampleName(id);sample(id)}}><span><Icon size={22}/></span><b>{title}</b><small>{subtitle}</small><i>Open sample <ArrowRight size={14}/></i></button>)}</div></section>
    <section id="features" className="landing-section landing-features"><div className="landing-section-heading"><span>BUILT FOR CLEARER ANSWERS</span><h2>A practical workspace for your data</h2></div><div className="feature-grid"><article><BarChart3/><h3>Automatic charts</h3><p>Get recommendations based on detected dates, categories, and measures.</p></article><article><Check/><h3>Data quality checks</h3><p>Review missing values, duplicates, suspicious values, and category inconsistencies.</p></article><article><Sparkles/><h3>Chart customization</h3><p>Choose chart types, fields, labels, filters, colors, and aggregation.</p></article><article><ArrowDownToLine/><h3>Useful exports</h3><p>Download charts, data files, and dashboard reports from your workspace.</p></article></div></section>
    <section className="landing-preview"><div className="landing-section-heading"><span>PREVIEW · SYNTHETIC SALES DATA</span><h2>See what your data can become</h2><p>These charts are generated from the bundled sample dataset.</p></div>{previewCharts.length?<div className="landing-preview-grid">{previewCharts.map((chart,index)=><ChartCard key={chart.title} data={preview} rec={chart} index={index}/>)}</div>:<div className="preview-loading">Loading the sample chart preview…</div>}</section>
    <section className="landing-saved">{savedDatasets.length>0&&<div className="recent-datasets"><div className="recent-heading"><div><b>Saved in your local workspace</b><span>Reopen an analyzed dataset from the local API</span></div><small>{savedDatasets.length} recent</small></div><div className="recent-list">{savedDatasets.map(item=>{const canOpen=item.processing_status==='ready'&&(item.rows??0)<=25000;return <div className="recent-item" key={item.dataset_id}><span className="recent-file"><FileSpreadsheet size={15}/></span><div><b>{item.filename}</b><small>{item.processing_status==='ready'?`${(item.rows??0).toLocaleString()} rows · ${item.columns??0} columns`:item.processing_status==='needs_sheet'?'Choose a worksheet in the original workbook':'Processing incomplete'}</small></div><button disabled={!canOpen||busy} onClick={()=>reopen(item)}>{canOpen?'Open':'Unavailable'} <ArrowRight size={13}/></button></div>})}</div></div>}</section>
  </div>;
}
